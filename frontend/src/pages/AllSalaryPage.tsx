...
import { LinkSheetRequest, OfficeHour } from "@/types/type"
...
export default function AllSalaryPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            const [data, configData] = await Promise.all([officeHoursService.getOfficeHoursByUser({ date: selectedConfig }), configService.getConfigDefault()])
            setConfigData(configData.linkSheet)
            setSelectedConfig(configData.linkSheet[0]?.month || "")
    ...
    useEffect(() => {
        ...
        const fetchAPI = async () => {
            const data = await salaryService.getSalaryByUser({ date: selectedConfig })
